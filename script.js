...
function setup() {
    ...
    projectileSound = loadSound('shoot.mp3');
    warpSound = loadSound('warp.mp3');
    gameOverSound = loadSound('over.mp3');

    music.setVolume(0.15);
    music.loop();

    projectileSound.setVolume(0.07);
...
function draw() {
    ...
    switch(gameState) {
        ...
        case 'play':
            ...
            if (player.transform.position.x < 0 || player.transform.position.x > screenSize.x ||
                player.transform.position.y < 0 || player.transform.position.y > screenSize.y) {
                player.framesOutsideBounds += 1;
            } else {
                player.framesOutsideBounds = 0;
            }

            if (player.framesOutsideBounds >= 6) {
                player.transform.position.x = 0.5 * screenSize.x;
                player.transform.position.y = 0.5 * screenSize.y;
            }
            ...
            if (player.isDestroyed == false) {
                ...
                // Run frame buffer
                if (gameOverFrameCount <= gameOverFrameBuffer) {
                    gameOverFrameCount += 1;
                
                // Once frame buffer is filled...
                } else {
                    gameState = 'end';
                }
        ...
        case 'menu':
            ...
            textSize(72);
            textAlign(CENTER);
            text('S.W.A.R.M.', 0.5 * screenSize.x, 0.3 * screenSize.y);
            stroke(230, 230, 230);
            strokeWeight(4);
            line(0.35 * screenSize.x, 0.34 * screenSize.y, 0.65 * screenSize.x, 0.34 * screenSize.y);
            strokeWeight(1);
            ...
            textSize(36);
            textAlign(CENTER);
            text('CONTROLS', 0.5 * screenSize.x, 0.45 * screenSize.y);
            ...
            textSize(36);
            textAlign(CENTER);
            text('PLAY', 0.5 * screenSize.x, 0.55 * screenSize.y);

            // Enemy model previews
            stroke(255, 102, 97);
            fill(255, 102, 97);

            // MANTARAY Model
            polyWheel(0.25 * screenSize.x, 0.49 * screenSize.y, [
                    1 * menuModelSize, 
                    0.52 * menuModelSize,
                    0.6 * menuModelSize,
                    0.3 * menuModelSize,
            ...
            // SPIKE Model
            polyWheel(0.75 * screenSize.x, 0.49 * screenSize.y, [
                    0.5 * menuModelSize, 
                    0.5 * menuModelSize,
                    0.5 * menuModelSize,
                    0.7 * menuModelSize,
        ...
        case 'controls':
            ...
            textSize(72);
            textAlign(CENTER);
            text('Controls', 0.5 * screenSize.x, 0.2 * screenSize.y);
            stroke(230, 230, 230);
            strokeWeight(4);
            line(0.35 * screenSize.x, 0.24 * screenSize.y, 0.65 * screenSize.x, 0.24 * screenSize.y);
            strokeWeight(1);
            ...
            textSize(36);
            textAlign(CENTER);
            text('W|S - Thrust: Forward|Backward', 0.5 * screenSize.x, 0.35 * screenSize.y);
            text('A|D - Rotate: Left|Right', 0.5 * screenSize.x, 0.45 * screenSize.y);
            text('Left Shift - Thuster Boost', 0.5 * screenSize.x, 0.55 * screenSize.y);
            text('Space - Shoot', 0.5 * screenSize.x, 0.65 * screenSize.y);

            // Back Button
            select('canvas').elt.style.letterSpacing = '4px';
            stroke(230, 230, 230);
            strokeWeight(4);
            line(0.35 * screenSize.x, 0.7 * screenSize.y, 0.65 * screenSize.x, 0.7 * screenSize.y);
            strokeWeight(1);
            textSize(42);
            text('BACK', 0.5 * screenSize.x, 0.8 * screenSize.y);
        ...
        case 'end':
            ...
            textSize(72);
            textAlign(CENTER);
            text('Game Over!', 0.5 * screenSize.x, 0.25 * screenSize.y);
            stroke(230, 230, 230);
            strokeWeight(4);
            line(0.35 * screenSize.x, 0.29 * screenSize.y, 0.65 * screenSize.x, 0.29 * screenSize.y);
            strokeWeight(1);   
            ...
            textSize(36);
            textAlign(CENTER);
            text('Score: ' + player.score, 0.5 * screenSize.x, 0.4 * screenSize.y);       
            ...
            textSize(36);
            textAlign(CENTER);
            text('Try Again', 0.5 * screenSize.x, 0.5 * screenSize.y);
            ...
            textSize(36);
            textAlign(CENTER);
            text('Main Menu', 0.5 * screenSize.x, 0.6 * screenSize.y);
...
function mouseClicked() {
    ...
    if (gameState == 'menu') {
        ...
        // Controls button
        if (mouseX < 0.6 * screenSize.x && mouseX > 0.4 * screenSize.x) {
            if (mouseY < 0.45 * screenSize.y && mouseY > 0.4 * screenSize.y) {
                gameState = 'controls';
            }
        }
        ...
        // Play button
        if (mouseX < 0.6 * screenSize.x && mouseX > 0.4 * screenSize.x) {
            if (mouseY < 0.55 * screenSize.y && mouseY > 0.5 * screenSize.y) {
                resetGame();
                gameState = 'play';
            }
        }
    ...
    if (gameState == 'controls') {
        if (mouseX < 0.6 * screenSize.x && mouseX > 0.4 * screenSize.x) {
            if (mouseY < 0.8 * screenSize.y && mouseY > 0.75 * screenSize.y) {
                gameState = 'menu';
            }
        }
    }
    ...
    if (gameState == 'end') {
        if (mouseX < 0.6 * screenSize.x && mouseX > 0.4 * screenSize.x) {
            if (mouseY < 0.5 * screenSize.y && mouseY > 0.45 * screenSize.y) {
                resetGame();
                gameState = 'play';
            }
        }
        if (mouseX < 0.6 * screenSize.x && mouseX > 0.4 * screenSize.x) {
            if (mouseY < 0.6 * screenSize.y && mouseY > 0.55 * screenSize.y) {
                gameState = 'menu';
            }
        }
    }
...
class Player {
    ...
    foldOverBounds() {  
        ...
        // If hits top-left screen bound, move to bottom-right screen bound, reset hasEnteredScreen
        ...
        // If hits bottom-left screen bound, move to top-right screen bound, reset hasEnteredScreen
        if (this.transform.position.x <= 0 && this.transform.position.y >= screenSize.y) { 
            this.transform.position.x = screenSize.x;
        ...
        // If hits top-right screen bound, move to bottom-left screen bound, reset hasEnteredScreen
        if (this.transform.position.x >= screenSize.x && this.transform.position.y <= 0) { 
        ...
        // If hits bottom-right screen bound, move to top-left screen bound, reset hasEnteredScreen
        if (this.transform.position.x >= screenSize.x && this.transform.position.y >= screenSize.y) { 
            this.transform.position.x = 0;
            this.transform.position.y = 0;
            this.hasEnteredScreen = false;
            return true;
...
class Enemy {
    ...
    constructor(type) {
        ...
        this.timeLastFired = 0;
        this.ambushRadius = random(280, 500);
    ...
    followPlayerAction() {
        ...
        if (this.distanceToPlayer >= this.ambushRadius + 25) {
            ...
            if (  // if next position will be off the screen, set velocity to zero
                this.transform.position.x + (-this.transform.direction.x * this.speed * dt) < 0 ||
                this.transform.position.x + (-this.transform.direction.x * this.speed * dt) > screenSize.x ||
                this.transform.position.y + (-this.transform.direction.y * this.speed * dt) < 0 ||
                this.transform.position.y + (-this.transform.direction.y * this.speed * dt) > screenSize.y 
            ) {
                this.transform.velocity.x = 0;
                this.transform.velocity.y = 0;
            } else {
    ...
    suicideFollow() {
        ...
        if (this.distanceToPlayer >= this.bomberMaxSpeed) { 
            this.transform.velocity.x = this.transform.direction.x * (this.speed / (this.distanceToPlayer / 800)); 
            this.transform.velocity.y = this.transform.direction.y * (this.speed / (this.distanceToPlayer / 800));
        } else {
            this.transform.velocity.x = this.transform.direction.x * (this.speed / (this.bomberMaxSpeed / 800)); 
            this.transform.velocity.y = this.transform.direction.y * (this.speed / (this.bomberMaxSpeed / 800));
        }
...
function explosion(i) {
    if (activeExplosions[i].frameCount <= explosionFrameBuffer) {
        ...
        fill(explosionColor);
        circle(activeExplosions[i].x, activeExplosions[i].y, 25 * (1 + 3 * activeExplosions[i].frameCount/explosionFrameBuffer));
        if (activeExplosions[i].frameCount == 0) {
            explosionSound.play();
        }
...
function spawnWave(wave) {
    ...
    for (let j = 0; j < random(int(pow(2, 0.15 * wave)), int(pow(2, 0.3 * wave))); j++) {
